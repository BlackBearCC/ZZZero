    ...
    async def _process_batch(self, batch_idx: int, batch_count: int, batch_topics: List[str], 
        ...
        for idx in range(batch_count):
            try:
                ...
                # 构建故事生成提示词
                system_prompt = f"""你是一个专业的对话故事创作助手，擅长生成高质量、情感丰富、适合与亲密的人分享的对话故事。

请根据提供的角色人设，创作一个精彩的短篇对话故事。
这个故事是专门用来与亲密的人分享的，应该能触发甜蜜美好的话题和情感共鸣，但故事本身不出现任何亲密的人的信息。
...
请确保输出为严格的JSON格式，禁止输出任何其他内容。"""
                ...
                user_message = Message(
                    ...
                    content=f"请创作一个适合与亲密的人分享的短篇对话故事"
                )
