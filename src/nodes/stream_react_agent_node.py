...
class StreamReactAgentNode(BaseNode):
    ...
    def _build_system_prompt(self, context: Any) -> str:
        ...
        # 专业AI助手ReAct提示词模板 - 强制Thought开头
        if tools_desc:
            base_prompt += "你是一个专业的AI助手，严格遵守ReAct推理模式。\n\n"
            base_prompt += "【重要规则】无论什么情况，你都必须以'Thought:'开头，这是不可违背的规则！\n\n"
            base_prompt += f"可用工具：\n{tools_desc}\n\n"
            base_prompt += "严格按照以下格式推理：\n"
            base_prompt += "Thought: [必须先思考！分析问题，制定解决方案]\n"
            base_prompt += f"Action: [选择工具，必须是 {', '.join(tool_names)} 中的一个]\n"
            base_prompt += "Action Input: [工具参数]\n"
            base_prompt += "Observation: [系统自动填充工具结果]\n"
            base_prompt += "Thought: [分析结果，决定是否需要继续]\n"
            base_prompt += "... (重复直到获得足够信息)\n"
            base_prompt += "Thought: [最终分析和总结]\n"
            base_prompt += "Final Answer: [给用户的最终答案]\n\n"
            base_prompt += "工作原则：\n"
            base_prompt += "1. 📋 每次回复都必须以'Thought:'开头\n"
            base_prompt += "2. 🛠️ 仔细分析问题，合理使用工具\n"
            base_prompt += "3. 🔍 在Observation后继续思考分析\n"
            base_prompt += "4. 🔄 如果信息不足，继续推理循环\n"
            base_prompt += "5. ✅ 确保答案完整准确\n"
            base_prompt += "6. 💡 充分利用历史信息\n"
